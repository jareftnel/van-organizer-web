...
"""
    ...
    if "</body>" in html and "wave-color-patch" not in html:
        html = html.replace(
            ...
            "    if(!timeLabel) return \"\";"
            "    var match = String(timeLabel).match(/(\\\\d{1,2}):(\\\\d{2})\\\\s*([AP]M)?/i);"
            "    if(!match) return \"\";"
            ...
            "  }"
            "  function getRoutes(){"
            "    if(typeof ROUTES !== \"undefined\") return ROUTES;"
            "    return window.ROUTES;"
            "  }"
            "  function getActiveRouteIndex(){"
            "    if(typeof activeRouteIndex === \"number\") return activeRouteIndex;"
            "    if(typeof window.activeRouteIndex === \"number\") return window.activeRouteIndex;"
            "    return 0;"
            "  }"
            "  function routeDisplayTitle(route){"
            "    if(!route) return \"\";"
            "    if(route.title) return String(route.title);"
            "    var short = route.route_short || \"\";"
            "    var cx = route.cx ? \" (\" + route.cx + \")\" : \"\";"
            "    return (short + cx).trim();"
            "  }"
            "  function findRouteTitleEl(){"
            "    return document.getElementById(\"routeTitle\")"
            "      || document.querySelector(\"[data-route-title]\")"
            "      || document.querySelector(\".routeTitle\")"
            "      || document.querySelector(\".route-title\");"
            "  }"
            "  function normalizeTimeLabel(label){"
            "    return String(label || \"\").trim();"
            "  }"
            "  function waveColorForLabel(label){"
            "    var key = timeKey(label);"
            "    if(key && waveColors[key]) return waveColors[key];"
            "    var key24 = timeKey24(label);"
            "    if(key24 && waveColors[key24]) return waveColors[key24];"
            "    var raw = normalizeTimeLabel(label);"
            "    if(raw && waveColors[raw]) return waveColors[raw];"
            "    var match = raw.match(/^(\\\\d{1,2})(?:[:.]?(\\\\d{2}))$/);"
            "    if(match){"
            "      var hh = match[1].padStart(2, \"0\");"
            "      var mm = match[2];"
            "      var compact = hh + \":\" + mm;"
            "      if(waveColors[compact]) return waveColors[compact];"
            "    }"
            "    return \"\";"
            "  }"
            "  function waveColorForRoute(route){"
            "    if(!route) return \"\";"
            "    return waveColorForLabel(route.wave_time || route.time_label || \"\");"
            "  }"
            "  function routeFromOption(opt, routes){"
            "    if(!opt || !routes) return null;"
            "    var idx = parseInt(opt.value, 10);"
            "    if(!Number.isNaN(idx) && routes[idx]) return routes[idx];"
            "    var valueText = String(opt.value || \"\").trim();"
            "    if(valueText){"
            "      for(var i = 0; i < routes.length; i++){"
            "        var route = routes[i];"
            "        var display = routeDisplayTitle(route);"
            "        if(display === valueText || String(route.title || \"\") === valueText) return route;"
            "      }"
            "    }"
            "    var labelText = String(opt.label || opt.text || \"\").trim();"
            "    if(labelText){"
            "      for(var j = 0; j < routes.length; j++){"
            "        var routeMatch = routes[j];"
            "        var displayMatch = routeDisplayTitle(routeMatch);"
            "        if(displayMatch === labelText || String(routeMatch.title || \"\") === labelText) return routeMatch;"
            "      }"
            "    }"
            "    return null;"
            "  }"
            "  function resolveActiveRoute(routes){"
            "    var idx = getActiveRouteIndex();"
            "    if(typeof idx === \"number\" && routes && routes[idx]) return routes[idx];"
            "    var selectEl = findRouteSelect();"
            "    if(selectEl && selectEl.options && selectEl.selectedIndex >= 0){"
            "      var opt = selectEl.options[selectEl.selectedIndex];"
            "      var fromOption = routeFromOption(opt, routes);"
            "      if(fromOption) return fromOption;"
            "    }"
            "    return routes && routes.length ? routes[0] : null;"
            "  }"
            "  function applyRouteWaveColor(){"
            "    var routeTitleEl = findRouteTitleEl();"
            "    var routes = getRoutes();"
            "    if(!routeTitleEl || !routes) return;"
            "    var route = resolveActiveRoute(routes) || {};"
            "    var color = waveColorForRoute(route);"
            "    if(color){"
              "      routeTitleEl.style.setProperty(\"color\", color, \"important\");"
              "      routeTitleEl.style.setProperty(\"text-decoration-line\", \"underline\", \"important\");"
              "      routeTitleEl.style.setProperty(\"text-decoration-color\", color, \"important\");"
              "      routeTitleEl.style.setProperty(\"text-decoration-thickness\", \"3px\", \"important\");"
            "    } else {"
            "      routeTitleEl.style.removeProperty(\"color\");"
            "      routeTitleEl.style.removeProperty(\"text-decoration-line\");"
            "      routeTitleEl.style.removeProperty(\"text-decoration-color\");"
            "      routeTitleEl.style.removeProperty(\"text-decoration-thickness\");"
            "    }"
            "  }"
            "  function findRouteSelect(){"
            "    return document.getElementById(\"routeSelect\")"
            "      || document.getElementById(\"routeSel\")"
            "      || document.querySelector(\"select[name='route']\")"
            "      || document.querySelector(\"select[id*='route']\")"
            "      || document.querySelector(\"select[id*='routeSel']\");"
            "  }"
            "  function applyRouteDropdownColors(){"
            "    var routes = getRoutes();"
            "    if(!routes) return;"
            "    var selectEl = findRouteSelect();"
            "    if(!selectEl || !selectEl.options) return;"
            "    Array.prototype.forEach.call(selectEl.options, function(opt){"
            "      var route = routeFromOption(opt, routes);"
            "      if(!route) return;"
            "      var color = waveColorForRoute(route);"
            "      if(color){"
            "        opt.style.setProperty(\"background-color\", color, \"important\");"
            "        opt.style.setProperty(\"color\", \"#0b0f14\", \"important\");"
            "        opt.dataset.waveColor = color;"
            "      } else {"
            "        opt.style.removeProperty(\"background-color\");"
            "        opt.style.removeProperty(\"color\");"
            "        if(opt.dataset) delete opt.dataset.waveColor;"
            "      }"
            "    });"
            "    Array.prototype.forEach.call(selectEl.querySelectorAll(\"optgroup\"), function(group){"
            "      var label = String(group.label || \"\");"
            "      var match = label.match(/\\(([^)]+)\\)/);"
            "      var timeLabel = match ? match[1] : label;"
            "      var groupColor = waveColorForLabel(timeLabel);"
            "      if(groupColor){"
            "        group.style.setProperty(\"background-color\", groupColor, \"important\");"
            "        group.style.setProperty(\"color\", \"#0b0f14\", \"important\");"
            "      } else {"
            "        group.style.removeProperty(\"background-color\");"
            "        group.style.removeProperty(\"color\");"
            "      }"
            "    });"
            "    var activeRoute = resolveActiveRoute(routes);"
            "    var activeColor = waveColorForRoute(activeRoute);"
            "    if(activeColor){"
            "      selectEl.style.setProperty(\"background-color\", activeColor, \"important\");"
            "      selectEl.style.setProperty(\"color\", \"#0b0f14\", \"important\");"
            "    } else {"

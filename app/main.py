...
"""
    ...
    if "</body>" in html and "combined-search-patch" not in html:
        html = html.replace(
            ...
            "    return s ? s.padStart(3,\"0\") : s;"
            "  }"
            "  // Extract tokens we want searchable from a card's text"
            "  function extractTokens(text){"
            "    const tokens = new Set();"
            "    const low = norm(text);"
            ""
            "    // Split on basics (keeps dots so 16.1b remains)"
            "    low.split(/[\\s,;\\/()]+/).forEach(t => { if(t) tokens.add(t); });"
            ""
            "    // Sort-zone like 16.1b (keep with dot)"
            "    (low.match(/\\b\\d{1,2}\\.\\d[a-z]\\b/g) || []).forEach(z => tokens.add(z));"
            ""
            "    // Overflow 99.* like 99.6y, 99.1m"
            "    (low.match(/\\b99\\.\\d[a-z]\\b/g) || []).forEach(z => tokens.add(z));"
            ""
            "    // All bare numbers (bag numbers, overflow counts, etc.)"
            "    (low.match(/\\b\\d{1,4}\\b/g) || []).forEach(n => {"
            "      tokens.add(n);"
            "      // Also add a zero-padded 3-digit variant so 94 → 094 queries match"
            "      if (n.length < 3) tokens.add(pad3(n));"
            "    });"
            ""
            ...
            "    cards.forEach(card => {"
            "      // Prefer explicit fields if present; fall back to innerText"
            "      const text = card.getAttribute('data-search-source')"
            "                 || card.innerText"
            "                 || card.textContent"
            "                 || \"\";"
            "      const toks = extractTokens(text);"
            "      card.dataset.search = toks.join(' ');"
            "    });"
            "    indexedVersion++;"
            "  }"
            ""

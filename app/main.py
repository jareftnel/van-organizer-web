...
"""
    ...
    if "</body>" in html and "wave-color-patch" not in html:
        html = html.replace(
            ...
            "      if(Number.isNaN(idx) || !routes[idx]) return;"
            "      var route = routes[idx] || {};"
            "      var key = timeKey(route.wave_time || route.time_label || \"\");"
            "      var color = key ? waveColors[key] : \"\";"
            "      if(color){"
            "        opt.style.backgroundColor = color;"
            "        opt.style.color = \"#0b0f14\";"
            "      } else {"
            ...
            "    selectEl.style.backgroundColor = selected ? selected.style.backgroundColor || \"\" : \"\";"
            "    selectEl.style.color = selected ? selected.style.color || \"\" : \"\";"
            "  }"

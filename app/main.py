...
"""
    ...
    if "</body>" in html and "overflow-controls-bottom-patch" not in html:
        html = html.replace(
            "</body>",
            "<style>"
            "/* overflow-controls-bottom-patch */"
            ".overflowControlsBar{"
            "  display:grid;"
            ...
            "  margin-top:12px;"
            "}"
            ".overflowControlsSlot{"
            "  display:flex;"
            "  align-items:center;"
            "}"
            ".overflowControlsSlot.left{justify-content:flex-start;}"
            ".overflowControlsSlot.center{justify-content:center;}"
            ".overflowControlsSlot.right{justify-content:flex-end;}"
            "</style>"
            "<script>"
            "/* overflow-controls-bottom-patch */"
            "(function(){"
            "  function $(sel, root){ return (root||document).querySelector(sel); }"
            "  function $all(sel, root){ return Array.prototype.slice.call((root||document).querySelectorAll(sel)); }"
            "  function findOverflowPanel(){"
            "    return $('#overflowPanel')"
            "      || $('[data-panel=\"overflow\"]')"
            "      || $('.overflowPanel')"
            "      || $('.panel.overflow');"
            "  }"
            "  function findOverflowTable(panel){"
            "    if(!panel) return null;"
            "    return panel.querySelector('table')"
            "      || panel.querySelector('.overflowTable')"
            "      || panel.querySelector('[data-overflow-table]')"
            "      || panel.querySelector('.tableWrap table')"
            "      || panel.querySelector('.tableContainer table');"
            "  }"
            "  function textContentOf(el){"
            "    return String((el && (el.textContent || el.getAttribute('aria-label') || el.getAttribute('title'))) || '').trim().toLowerCase();"
            "  }"
            "  function findButtonByText(panel, label){"
            "    var buttons = $all('button, [role=\"button\"]', panel);"
            "    var match = null;"
            "    buttons.some(function(btn){"
            "      var text = textContentOf(btn);"
            ...
            "    return match;"
            "  }"
            "  function isToggleGroup(el){"
            "    if(!el) return false;"
            "    var buttons = $all('button, [role=\"button\"]', el);"
            "    if(buttons.length < 2) return false;"
            "    var labels = buttons.map(textContentOf).join(' ');"
            "    return /(table|list|grid|card|cards|view)/.test(labels);"
            "  }"
            "  function findViewToggles(panel){"
            "    var selectors = ["
            "      '[data-view-toggle]',"
            "      '.viewToggle',"
            "      '.viewToggles',"
            ...
            "      '.viewToggleRow'"
            "    ];"
            "    var candidates = $all(selectors.join(','), panel);"
            "    for(var i=0;i<candidates.length;i++){"
            "      if(isToggleGroup(candidates[i])) return candidates[i];"
            "    }"
            "    var groups = $all('div, nav, section', panel);"
            "    for(var j=0;j<groups.length;j++){"
            "      if(isToggleGroup(groups[j])) return groups[j];"
            "    }"
            "    return null;"
            "  }"
            "  function ensureControlsBar(panel, table){"
            "    var bar = panel.querySelector('.overflowControlsBar');"
            "    if(!bar){"
            "      bar = document.createElement('div');"
            "      bar.className = 'overflowControlsBar';"
            "      var left = document.createElement('div');"
            "      left.className = 'overflowControlsSlot left';"
            "      var center = document.createElement('div');"
            "      center.className = 'overflowControlsSlot center';"
            "      var right = document.createElement('div');"
            "      right.className = 'overflowControlsSlot right';"
            ...
            "    return bar;"
            "  }"
            "  function normalizeTopSpacing(panel, table){"
            "    if(panel){ panel.style.paddingTop = '0'; }"
            "    if(table){ table.style.marginTop = '0'; }"
            "    if(table && table.parentElement){"
            "      table.parentElement.style.marginTop = '0';"
            "      table.parentElement.style.paddingTop = '0';"
            "    }"
            "  }"
            "  function collapseEmptyContainer(container){"
            "    if(!container) return;"
            ...
            "    var panel = findOverflowPanel();"
            "    if(!panel) return;"
            "    var table = findOverflowTable(panel);"
            "    if(!table) return;"
            "    var bar = ensureControlsBar(panel, table);"
            "    var viewToggles = findViewToggles(panel);"
            "    var clearBtn = findButtonByText(panel, 'clear');"
            "    var syncBtn = findButtonByText(panel, 'sync');"
            "    var leftSlot = bar.querySelector('.overflowControlsSlot.left');"
            "    var centerSlot = bar.querySelector('.overflowControlsSlot.center');"
            "    var rightSlot = bar.querySelector('.overflowControlsSlot.right');"
            "    if(viewToggles && !viewToggles.closest('.overflowControlsBar')){"
            "      var oldParent = viewToggles.parentElement;"
            "      leftSlot.appendChild(viewToggles);"
            "      collapseEmptyContainer(oldParent);"
            "    }"
            "    if(syncBtn && !syncBtn.closest('.overflowControlsBar')){"
            "      var oldSyncParent = syncBtn.parentElement;"
            "      centerSlot.appendChild(syncBtn);"
            "      collapseEmptyContainer(oldSyncParent);"
            "    }"
            "    if(clearBtn && !clearBtn.closest('.overflowControlsBar')){"
            "      var oldClearParent = clearBtn.parentElement;"
            "      rightSlot.appendChild(clearBtn);"
            "      collapseEmptyContainer(oldClearParent);"
            "    }"
            "    normalizeTopSpacing(panel, table);"
            "  }"
            "  document.addEventListener('click', function(ev){"
            "    var target = ev.target;"
            "    if(target && target.classList && target.classList.contains('tab')){"
            "      setTimeout(moveControls, 0);"
            "    }"
            "  });"
            "  window.addEventListener('load', moveControls);"
            "  var observer = new MutationObserver(function(){ moveControls(); });"
